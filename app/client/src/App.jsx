...
function App() {
  ...
  const loadJobs = async () => { try { setJobs(await getJobs()); setError('') } catch { setError('Could not reach the API. Check that the server is running.') } finally { setLoading(false) } }
  useEffect(() => {
    let active = true
    getJobs().then((items) => { if (active) setJobs(items) }).catch(() => { if (active) setError('Could not reach the API. Check that the server is running.') }).finally(() => { if (active) setLoading(false) })
    return () => { active = false }
  }, [])
